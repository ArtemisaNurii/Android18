...
// --- Interfaces and Data (Added a `features` array) ---
interface Project {
  id: number;
  title: string;
  category: string;
  ...
  description: string;
  link: string;
  features: string[]; // <-- Crucial addition for technical credibility
}
...
const projectsData: Project[] = [
    ...
    { 
      ...
      link: '/projects/corporate-branding',
        ...
        const ctx = gsap.context(() => {
            if (showcaseRef.current) {
                gsap.from(showcaseRef.current.children, {
                    ...
                    duration: 0.8,
                    ease: 'power3.out',
                    stagger: 0.2, // Animate image and text block separately
                    scrollTrigger: {
    ...
    return (
        <div ref={showcaseRef} className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className={`group relative ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                <div className="absolute -inset-2.5 bg-gradient-to-r from-emerald-600 to-cyan-500 rounded-xl opacity-0 group-hover:opacity-75 transition duration-500 blur-xl"></div>
                <img 
...
">
            <div className="absolute inset-0 bg-gradient-to-br from-black to-teal-300 text-white z-0" />
